	...
	public class TabControl : Panel
	{
		...
		protected virtual void Recalculate()
		{
			...
			if (hideTabsArea)
			{
				...
				_pageAreaRect = new Rectangle(0, 0, Width, Height);
			...
			else
			{
				if (_positionAtTop)
				{
					...
					_pageAreaRect = new Rectangle(0, tabsAreaHeight, Width, Height - tabsAreaHeight);

					// Create rectangle that represents the entire area for pages
					_tabsAreaRect = new Rectangle(0, 0, Width, tabsAreaHeight);
				}
				else
				{
					// Create rectangle that represents the entire tabs area
					_tabsAreaRect = new Rectangle(0, Height - tabsAreaHeight, Width, tabsAreaHeight);

					// Create rectangle that represents the entire area for pages
					_pageAreaRect = new Rectangle(0, 0, Width, Height - tabsAreaHeight);
				}
			}
		...
		protected virtual void RecalculateButtons()
		{
			...
			if ((_appearance == VisualAppearance.MultiBox) || (_style == VisualStyle.Plain))
				_closeButton.BackColor = _leftArrow.BackColor = _rightArrow.BackColor = BackColor;
			else
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			using (SolidBrush pageAreaBrush = new SolidBrush(BackColor))
			{
					...
					case VisualStyle.Plain:
						...
						if (_insetBorderPagesOnly)
						{
							if (!hiddenPages)
							{
								...
								DrawHelper.DrawPlainRaisedBorderTopOrBottom(e.Graphics, new Rectangle(0, xDraw, Width, pageHeight),
						...
						{
							// Draw the outer border around the page area			
							DrawHelper.DrawPlainRaisedBorder(e.Graphics,
							                                 new Rectangle(_pageAreaRect.Left, xDraw, _pageAreaRect.Width, pageHeight),
							                                 _backLightLight, base.BackColor, _backDark, _backDarkDark);
						}
						...
						if ((_tabPages.Count > 0) && _insetPlain)
						{
							...
							if (_insetBorderPagesOnly)
							{
								if (!hiddenPages)
								{
									DrawHelper.DrawPlainSunkenBorderTopOrBottom(e.Graphics, new Rectangle(0, inner.Top, Width, inner.Height),
									                                            _backLightLight, base.BackColor, _backDark, _backDarkDark,
									                                            _positionAtTop);
								}
							}
					...
					case VisualStyle.IDE:
						           ...
						           backColor = new Pen(base.BackColor))
						{
							...
							if (_positionAtTop)
							{
								...
								if (_idePixelBorder)
								{
									using (Pen llFore = new Pen(ControlPaint.LightLight(ForeColor)))
		...
		protected virtual void RemoveTabPageControl(Control c)
		{
			...
			if (controlIsForm == null)
			{
				...
				ControlHelper.Remove(Controls, c);
			}
			else
			{
		...
		protected virtual void RemoveTabPagePanel(TabPage page)
		{
			...
			// Use helper method to circumvent form Close bug
			ControlHelper.Remove(Controls, page);
		}
		...
		protected override void OnDoubleClick(EventArgs e)
		{
			...
			for (int index = 0; index < count; index++)
			{
				...
				if (local != _nullPosition)
				{
					...
					Rectangle screen = RectangleToScreen(local);
					...
					if (_tabsAreaRect.Contains(mousePos))
					{
						...
						if (!ce.Cancel)
						{
							...
							if (_contextMenu != null)
								_contextMenu.TrackPopup(PointToScreen(new Point(e.X, e.Y)));
						}
					}
				}
			}
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			if (_leftMouseDown)
			{
				if (!_leftMouseDownDrag)
				{
					...
					if (_dragFromControl)
						startDrag = !ClientRectangle.Contains(thisPosition);
					else
					{
