...
namespace Crownwood.Magic.Controls
{
	...
	public class ResizeBar : Control
	{
		...
		public ResizeBar(Direction direction, IResizeSource resizeSource)
		{
			...
			BackColor = _resizeSource.ResizeBarColor;
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			switch (Dock)
			{
				case DockStyle.Right:
					...
					{
						e.Graphics.DrawLine(penD, Width - 2, 0, Width - 2, Height);
						e.Graphics.DrawLine(penDD, Width - 1, 0, Width - 1, Height);
					}
					break;
				case DockStyle.Left:
					using (Pen penLL = new Pen(ControlPaint.LightLight(backColor)))
						e.Graphics.DrawLine(penLL, 1, 0, 1, Height);
					break;
				case DockStyle.Bottom:
					using (Pen penD = new Pen(ControlPaint.Dark(backColor)),
					           penDD = new Pen(ControlPaint.DarkDark(backColor)))
					{
						e.Graphics.DrawLine(penD, 0, Height - 2, Width, Height - 2);
						e.Graphics.DrawLine(penDD, 0, Height - 1, Width, Height - 1);
					}
					break;
				case DockStyle.Top:
					using (Pen penLL = new Pen(ControlPaint.LightLight(backColor)))
						e.Graphics.DrawLine(penLL, 0, 1, Width, 1);
					break;
			}
		}
	}
