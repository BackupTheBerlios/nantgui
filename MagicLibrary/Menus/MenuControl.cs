...
namespace Crownwood.Magic.Menus
{
	...
	public class MenuControl : ContainerControl, IMessageFilter
	{
		...
		public void ResetColors()
		{
			...
			SelectedBackColor = SystemColors.Control;
			SelectedTextColor = SystemColors.MenuText;
		}
		...
		protected void DefineFont(Font newFont)
		{
			...
			_breadthGap = (Font.Height/3) + 1;

			// Calculate the initial height/width of the control
			_rowWidth = _rowHeight = Font.Height + _breadthGap*2 + 1;
		}
		...
		protected void CreatePendantButtons()
		{
			...
			// Add to display
			Controls.AddRange(new Control[] {_minButton, _restoreButton, _closeButton});
		}
		...
		protected void RemovePendantButtons()
		{
			...
			ControlHelper.Remove(Controls, _minButton);
			ControlHelper.Remove(Controls, _restoreButton);
			ControlHelper.Remove(Controls, _closeButton);
		...
		internal void OnWM_MOUSEDOWN(POINT screenPos)
		{
			...
			User32.ScreenToClient(Handle, ref screenPos);
				...
				if (dc.SelectRect.Contains(pos) && dc.Enabled)
				{
					...
					else
					{
						...
						if (dc.Chevron || (dc.MenuCommand.MenuCommands.Count > 0))
							User32.PostMessage(Handle, WM_OPERATEMENU, 1, 0);
					}
		...
		protected void Recalculate()
		{
			...
			if (length > 0)
			{
				...
				if (showPendant)
				{
					...
					if (_direction == Direction.Horizontal)
					{
						xPos = Width - _pendantOffset - _buttonLength;
						yPos = _pendantOffset;
						xInc = -_buttonLength;
					}
					else
					{
						xPos = _pendantOffset;
						yPos = Height - _pendantOffset - _buttonLength;
						yInc = -_buttonLength;
					}
				}
				...
				using (Graphics g = CreateGraphics())
				{
					...
					foreach (MenuCommand command in _menuCommands)
					{
						...
						else
						{
							...
							SizeF dimension = g.MeasureString(command.Text, Font);
						...
						if ((lengthStart > length) && (columns > 1))
						{
							...
							else
							{
								...
								if (_direction == Direction.Horizontal)
								{
									...
									cellRect.X = Width - cellRect.Width;
								...
								else
								{
									...
									cellRect.Y = Height - (cellMinLength + _chevronLength);
			...
			if (!dc.Separator)
			{
						...
						switch (_style)
						{
										...
										if (dc.SubMenu)
										{
												...
												if (_supportsLayered && (_style == VisualStyle.IDE))
												{
													using (
														LinearGradientBrush topBrush =
															new LinearGradientBrush(new Point(rightLeft - _shadowGap, rightTop + _shadowGap),
											...
											else
											{
												...
												if (_supportsLayered && (_style == VisualStyle.IDE))
												{
													using (
														LinearGradientBrush topBrush =
															new LinearGradientBrush(new Point(leftLeft + _shadowGap, rightBottom + 1 - _shadowGap),
												...
												g.FillRectangle(shadowBrush,
												                new Rectangle(leftLeft, rightBottom + 1, rightBottom - leftLeft - _shadowGap, _shadowGap));
							...
							case VisualStyle.Plain:
								...
								else
								{
									if (_selected)
									{
										using (Pen lighlight = new Pen(ControlPaint.LightLight(BackColor)),
				...
				else
				{
					...
					if (dc.Enabled && Enabled)
					{
						if (tracked && (_style == VisualStyle.Plain) && _plainAsBlock)
						{
							...
							using (SolidBrush textBrush = new SolidBrush(_plainSelectedTextColor))
								g.DrawString(mc.Text, Font, textBrush, textRect, format);
						}
						else
						{
					...
					else
					{
						...
						g.DrawString(mc.Text, Font, Brushes.White, rectDownRight, format);

						// Draw then text offset up and left
						using (SolidBrush grayBrush = new SolidBrush(SystemColors.GrayText))
							g.DrawString(mc.Text, Font, grayBrush, textRect, format);
					}
				}
			}
		}
				...
				for (int i = 0; i < _drawCommands.Count; i++)
				{
					...
					if (!dc.Separator && (dc.Chevron || dc.MenuCommand.Enabled))
					{
						...
						if (newItem != _trackItem)
						{
							...
							{
								if (dc.Chevron || (dc.MenuCommand.MenuCommands.Count > 0))
									User32.PostMessage(Handle, WM_OPERATEMENU, 0, 1);
							}
