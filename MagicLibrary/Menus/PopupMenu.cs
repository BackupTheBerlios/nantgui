	...
	public class PopupMenu : NativeWindow
	{
		...
		public void Dismiss()
		{
			if (Handle != IntPtr.Zero)
			{
				...
				User32.PostMessage(Handle, WM_DISMISS, 0, 0);
			}
		}

		protected void HideMenuWindow()
		{
			User32.ShowWindow(Handle, (short) ShowWindowStyles.SW_HIDE);
		}
		...
		protected void CreateAndShowWindow()
		{
			...
			if (_layered)
			{
				...
				if (animate && _animateIn)
				{
					...
					{
						// Animate the appearance of the window
						User32.AnimateWindow(Handle, (uint) _animateTime, animateFlags);
					}
		...
		protected void SetWindowRegion(Size winSize)
		{
			...
			if (_style == VisualStyle.IDE)
			{
				...
				using (Graphics g = Graphics.FromHwnd(Handle))
					User32.SetWindowRgn(Handle, drawRegion.GetHrgn(g), false);
			}
		}
		...
		protected void RegenerateExpansion()
		{
			...
			if (!_layered)
			{
				...
				User32.MoveWindow(Handle, newPos.X, newPos.Y, newSize.Width, newSize.Height, true);
				...
				User32.InvalidateRect(Handle, ref clientRect, true);
			}
			else
			{
				// Update the image for display
		...
		protected void RefreshAllCommands()
		{
			...
			User32.GetWindowRect(Handle, ref rectRaw);
				...
				case VisualStyle.IDE:
					...
					if ((_borderGap > 0) && (!_excludeTop) && (_direction == Direction.Horizontal))
					{
						...
						if (xExcludeStart >= leftLeft)
							DrawShadowHorizontal(g, leftLeft, rightBottom, xExcludeStart - leftLeft,
							                     _position[(int) _style, (int) PI.ShadowHeight], Shadow.Left);

						if (xExcludeEnd <= rightright)
							DrawShadowHorizontal(g, xExcludeEnd, rightBottom, rightright - xExcludeEnd,
							                     _position[(int) _style, (int) PI.ShadowHeight], Shadow.Right);
					}
					else
					{
						if ((_direction == Direction.Vertical) && (!_excludeTop))
						...
						DrawShadowHorizontal(g, leftLeft, rightBottom, rightLeft, _position[(int) _style, (int) PI.ShadowHeight],
						                     Shadow.All);
					}

					DrawShadowVertical(g, rightLeft, rightTop, _position[(int) _style, (int) PI.ShadowWidth],
			...
			if (_layered)
			{
				...
				if (op != Shadow.Right)
				{
					if (width >= _shadowLength)
					{
						...
						using (
							LinearGradientBrush leftBrush = new LinearGradientBrush(new Point(left + _shadowLength, top - _shadowLength),
					...
					else
					{
						switch (_style)
						{
							case VisualStyle.IDE:
								...
								g.FillRectangle(_controlLLBrush,
								...
								if (dc.Infrequent && _highlightInfrequent)
								{
									...
									g.FillRectangle(Brushes.White,
									                new Rectangle(leftPos, drawRect.Top, drawRect.Right - leftPos - textGapLeft, drawRect.Height));
									g.FillRectangle(_controlEBrush,
									                new Rectangle(leftPos, drawRect.Top, drawRect.Right - leftPos - textGapLeft, drawRect.Height));
								}
		...
		{
			User32.PostMessage(Handle, WM_OPERATE_SUBMENU, (uint) popupItem, (uint) (selectFirst ? 1 : 0));
		}
		...
		protected void OnWM_OPERATE_SUBMENU(ref Message m)
		{
			...
			User32.ClientToScreen(Handle, ref screenPosTR);

			// Find screen coordinate of top left of item cell
			POINT screenPosTL;
			screenPosTL.x = dc.DrawRect.Left;
			screenPosTL.y = dc.DrawRect.Top;
			User32.ClientToScreen(Handle, ref screenPosTL);

			// Ensure the child has the same properties as ourself
			_childMenu.Style = Style;
			_childMenu.Font = Font;
			_childMenu.BackColor = BackColor;
			_childMenu.TextColor = TextColor;
			_childMenu.HighlightTextColor = HighlightTextColor;
			_childMenu.HighlightColor = HighlightColor;
			_childMenu.Animate = Animate;
			_childMenu.AnimateStyle = AnimateStyle;
			_childMenu.AnimateTime = AnimateTime;
		...
		protected bool OnWM_NCHITTEST(ref Message m)
		{
			...
			if (_style == VisualStyle.IDE)
			{
				...
				// Convert the mouse position to screen coordinates
				User32.ClientToScreen(Handle, ref popupPos);
...
//			Console.WriteLine("WndProc(PopupMenu) {0} {1}", this.Handle, ((Win32.Msgs)m.Msg).ToString());
		...
		protected static Bitmap GetShadowCache(int width, int height)
		{
			...
			BitmapData data =
				image.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.WriteOnly, PixelFormat.Format32bppArgb);
