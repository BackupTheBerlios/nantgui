	...
	public class WindowDetailCaption : WindowDetail, IMessageFilter
	{
			...
			: base(manager)
		{
			...
			if (invertAutoHideHandler != null)
				InvertAutoHide += invertAutoHideHandler;
		...
		protected void RecalculateMaximizeButton()
		{
			...
			if (ParentZone != null)
			{
				...
				IZoneMaximizeWindow zmw = ParentZone as IZoneMaximizeWindow;
		...
		protected void RemoveMaximizeInterface()
		{
			if (_maxInterface != null)
			{
				...
				ControlHelper.Remove(Controls, _maxButton);
		...
		protected void OnButtonMax(object sender, EventArgs e)
		{
			if (ParentWindow != null)
			{
				if (_maxInterface.IsMaximizeAvailable())
				{
					...
					if (_maxInterface.IsWindowMaximized(ParentWindow))
						_maxInterface.RestoreWindow();
					else
						_maxInterface.MaximizeWindow(ParentWindow);
				}
			}
		}
		...
		protected void OnButtonGotFocus(Object sender, EventArgs e)
		{
			...
			if (ParentWindow != null)
				ParentWindow.WindowDetailGotFocus(this);
		}
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			...
			if (!IsDisposed)
			{
				...
				else
				{
					...
					{
						if (ParentWindow.RedockAllowed)
						{
							WindowContent wc = ParentWindow as WindowContent;
	...
	public class WindowDetailCaptionPlain : WindowDetailCaption
	{
		...
		protected override void UpdateMaximizeImage()
		{
			...
			if (_maxButton != null)
			{
				...
				bool maximized = _maxInterface.IsWindowMaximized(ParentWindow);
		...
		protected override void RecalculateButtons()
		{
			if (_dockLeft)
			{
				if (Dock != DockStyle.Left)
				{
					RemovedFromParent(ParentWindow);
					Dock = DockStyle.Left;
					AddedToParent(ParentWindow);
				}
	...
	public class WindowDetailCaptionIDE : WindowDetailCaption
	{
		...
		protected override void OnAddMaximizeInterface()
		{
			if (_maxButton != null)
			{
				...
				_maxButton.BackColor = BackColor;
				_maxButton.ImageAttributes = _inactiveAttr;
		...
		protected void SetButtonState()
		{
			if (ParentWindow != null)
			{
				...
				{
					if (_closeButton.BackColor != BackColor)
					{
					...
					if (_hideButton != null)
					{
						if (_hideButton.BackColor != BackColor)
						{
							_hideButton.BackColor = BackColor;
							_hideButton.ImageAttributes = _inactiveAttr;
							_hideButton.Invalidate();
						}
					}
					...
					if (_maxButton != null)
					{
						if (_maxButton.BackColor != BackColor)
						{
							_maxButton.BackColor = BackColor;
							_maxButton.ImageAttributes = _inactiveAttr;
							_maxButton.Invalidate();
		...
		protected override void RecalculateButtons()
		{
			int buttonX = Width - _buttonWidth - _buttonSpacer;
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if (ParentWindow != null)
				focused = ParentWindow.ContainsFocus;
			                  ...
			                  inactiveBrush = new SolidBrush(_manager.InactiveTextColor))
			{
				...
				if (focused)
				{
					...
					e.Graphics.DrawString(Text, _manager.CaptionFont, activeTextBrush, rectCaption);
				}
				else
				{
					// Fill the entire background area
					...
					using (Pen dark = new Pen(ControlPaint.LightLight(_manager.InactiveTextColor)))
					{
						...
						e.Graphics.DrawString(Text, _manager.CaptionFont, inactiveBrush, rectCaption);
					}
				}
			}
