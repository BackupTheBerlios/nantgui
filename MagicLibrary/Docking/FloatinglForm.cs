...
namespace Crownwood.Magic.Docking
{
	public class FloatingForm : Form, IHotZoneSource, IMessageFilter
	{
		...
		public FloatingForm(DockingManager dockingManager, Zone zone, ContextHandler contextHandler)
		{
			...
			Owner = dockingManager.Container.FindForm();

			// Need to know when the Zone is removed
			ControlRemoved += new ControlEventHandler(OnZoneRemoved);
			...
			// Default color
			BackColor = _dockingManager.BackColor;
			ForeColor = _dockingManager.InactiveTextColor;
			...
			if (_zone.Windows.Count == 1)
			{
				...
				Text = _zone.Windows[0].FullTitle;
			}
		...
		public Zone Zone
		{
			get { return Controls[0] as Zone; }
		}

		public void PropogateNameValue(PropogateName name, object value)
		{
			if (Zone != null)
				Zone.PropogateNameValue(name, value);
		}
		...
		protected override void OnResize(EventArgs e)
		{
			...
			Size newSize = new Size(Width, Height - SystemInformation.ToolWindowCaptionHeight);
		...
		protected override void WndProc(ref Message m)
		{
			...
			else if (m.Msg == (int) Msgs.WM_NCLBUTTONDOWN)
			{
				if (!_intercept)
				{
					...
					uint result = User32.SendMessage(Handle, (int) Msgs.WM_NCHITTEST, 0, (uint) m.LParam);
