	...
	public class AutoHidePanel : Panel, IMessageFilter
	{
		...
		protected class AutoHostPanel : Panel, IResizeSource
		{
			...
			public bool StartResizeOperation(ResizeBar bar, ref Rectangle screenBoundary)
			{
				...
				screenBoundary = Parent.RectangleToScreen(_manager.InnerResizeRectangle(this));

				// Find the screen limits of this Zone
				Rectangle panelBoundary = RectangleToScreen(ClientRectangle);
		...
		public Restore RestoreObjectForContent(Content c)
		{
			...
			{
				TabStub ts = Controls[controlIndex] as TabStub;
		...
		public void AddContentsAsGroup(ContentCollection contents, int index)
		{
			...
			WindowContentTabbed wct = _manager.CreateWindowForContent(null, new EventHandler(OnPageClose),
			                                                          ...
			                                                          new ContextHandler(OnPageContextMenu)) as
			                          WindowContentTabbed;
		...
		protected void OnPageOver(TabStub sender, int pageIndex)
		{
			...
			{
				case DockStyle.Left:
					_currentPanel.Size = new Size(0, Height);
					_currentPanel.Location = new Point(Right, Top);
					_currentWCT.Height = Height;
					break;
				case DockStyle.Right:
					_currentPanel.Size = new Size(0, Height);
					_currentPanel.Location = new Point(Left, Top);
					_currentWCT.Height = Height;
					break;
				case DockStyle.Top:
					_currentPanel.Size = new Size(Width, 0);
					_currentPanel.Location = new Point(Left, Bottom);
					_currentWCT.Width = Width;
					break;
				case DockStyle.Bottom:
					_currentPanel.Size = new Size(Width, 0);
					_currentPanel.Location = new Point(Left, Top);
		...
		public void RemoveContent(Content c)
		{
			...
			if ((targetTS != null) && (targetPage != null))
			{
				...
				if (targetTS.TabPages.Count == 1)
				{
					...
					ControlHelper.Remove(Controls, targetTS);
				...
				if (ts.WindowContentTabbed == _currentWCT)
				{
					...
					if (!_manager.OnContentHiding(current))
					{
						...
						if (ts.TabPages.Count == 1)
						{
							...
							// Remove the stub from this panel
							ControlHelper.Remove(Controls, ts);
		...
		protected void OnSlideTick(object sender, EventArgs e)
		{
			...
			if (_slideRunning)
			{
				...
				if (IsDisposed || _currentPanel.IsDisposed || _currentWCT.IsDisposed)
				{
					StopSlideTimer();
					return;
				}
		...
		{
			// Safety check that timer does not expire after our death
			if (IsDisposed || (_currentPanel == null) || _currentPanel.IsDisposed)
			{
				StopDismissTimer();
				return;
			}
			    ...
			    !_currentPanel.IsDisposed)
			{
				switch (msg.Msg)
				{
					case (int) Msgs.WM_MOUSEMOVE:
						...
						Rectangle thisRect = RectangleToScreen(ClientRectangle);
