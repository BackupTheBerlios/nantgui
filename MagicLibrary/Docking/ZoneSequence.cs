	...
	public class ZoneSequence : Zone, IHotZoneSource, IZoneMaximizeWindow, IResizeSource
	{
		...
		public bool StartResizeOperation(ResizeBar bar, ref Rectangle screenBoundary)
		{
			...
			if (bar == _resizeBar)
			{
				...
				Rectangle zoneBoundary = RectangleToScreen(ClientRectangle);
			...
			else
			{
				...
				screenBoundary = RectangleToScreen(ClientRectangle);
		...
		public void AddHotZones(Redocker redock, HotZoneCollection collection)
		{
			...
			Rectangle zoneArea = RectangleToScreen(ClientRectangle);
		...
		protected override void OnWindowRemoving(int index, object value)
		{
			...
			if (_windows.Count == 1)
			{
				...
				ControlHelper.RemoveAt(Controls, 0);
			}
			else
			{
				int pos = 0;
				...
				// Use helper method to circumvent form Close bug
				ControlHelper.RemoveAt(Controls, pos);
				ControlHelper.RemoveAt(Controls, pos);
			}
		...
		protected void RepositionZoneBar(ref Rectangle clientRect)
		{
			...
			bool resizeBarPresent = ((Dock != DockStyle.Fill) && (Dock != DockStyle.None));
			...
			switch (_state)
			{
				case State.DockLeft:
					...
					barRect = new Rectangle(Width - length, 0, length, Height);
				...
				case State.DockTop:
					...
					barRect = new Rectangle(0, Height - length, Width, length);
				...
				case State.DockRight:
					_resizeBar.Direction = Direction.Horizontal;
					barRect = new Rectangle(0, 0, length, Height);
				...
				case State.DockBottom:
					_resizeBar.Direction = Direction.Vertical;
					barRect = new Rectangle(0, 0, Width, length);
